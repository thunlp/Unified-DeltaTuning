...
from modeling_t5_PET_mode_connectivity import (
    # T5Model,
    MyT5_pet_MC,
)
...
from t5_trainer_PET_mc import Trainer
...
def main():
    ...
    parser.add_argument("--low_dimension", type=int, default=10)
    parser.add_argument("--narrow_P", action='store_true')
    parser.add_argument("--decoder_type", type=int, default=1)
    parser.add_argument("--reconstruct_alpha", type=float, default=0.99)
    ...
    for prefix in prefixes:
        ...
        for bsz in args.bsz_list:
            for lr in args.learning_rate_list:
                ...
                if args.do_predict:
                    metric_key, test_performance = trainer.test()

                
                logger.info("prefix={}, lr={}, bsz={}, dev_performance={}, test_performance={}".format(prefix, lr, bsz, dev_performance, test_performance))
                df.loc[len(df.index)] = [prefix, metric_key, lr, bsz, dev_performance, test_performance]
