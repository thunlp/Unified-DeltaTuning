...
def fastfood_torched(x, DD: int, param_list: Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, int]):
    ...
    # test_dd_pad = FastWalshHadamard.apply(mul_2) / LL

    # HG(PiHBX)
    mul_3 = mul_2[Pi]
    # inverse_Pi = torch.LongTensor(np.empty(LL, dtype=np.int32))
    # inverse_Pi[Pi] = torch.arange(0, LL, 1, out=None)

    # test_inverse_Pi = mul_3[inverse_Pi]
...
class IntrinsicDimensionLight:
    def __init__(self, module: nn.Module, intrinsic_dimension: int,  output_dir,
        ...
        if not (module.share_intrinsic == None):
            # self.share_intrinsic = module.share_intrinsic.cpu() if device=="cpu" else module.share_intrinsic.cuda()
            self.share_intrinsic = module.share_intrinsic
        ...
        for name, param in module.named_parameters():
            ...
            if 'adapter' not in name and 'lora' not in name and 'prefix' not in name and (len(str_filter) == 0 or any([x in name for x in str_filter])):
                length += 1
                self.initial_value[name] = v0 = (
                    param.clone().detach().requires_grad_(False).to(self.share_intrinsic.device)
                )
                DD = np.prod(v0.size())
                self.projection_params[name] = self.get_projection_params(DD, self.share_intrinsic.device)
                if self.projection == 'fastfood':
                    new_param = nn.Parameter(self.projection_params[name][2])
                    self.projection_params[name][2] = new_param
                    module.register_parameter("trained_said_" + '_'.join(name.split('.')), new_param)
                    setattr(module,"trained_said_" + '_'.join(name.split('.')), new_param)
                elif self.projection == 'random':
                    for idx in range(2):
                        new_param = nn.Parameter(self.projection_params[name][idx])
                        self.projection_params[name][idx] = new_param
                        module.register_parameter("trained_said_" + '_'.join(name.split('.')) + '_' + str(idx), new_param)
                        setattr(module, "trained_said_" + '_'.join(name.split('.')) + '_' + str(idx), new_param)
    ...
    def __call__(self, module, inputs, kwargs):
        index = 0
        if kwargs['only_adapter'] or kwargs['only_lora'] or kwargs['only_prefix']:
            if kwargs['only_said']:
                assert False
            else:
            ...
            for name, base, localname in self.name_base_localname:
                ...
                if self.device == "cpu":
                    self.projection_params[name] = self.move_to(
                        self.projection_params[name], module.share_intrinsic.device)
                ray = self.get_projected_param(module.share_intrinsic, DD, self.projection_params[name], init_shape)
