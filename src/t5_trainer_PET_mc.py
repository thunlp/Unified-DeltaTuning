...
class Trainer:
    def __init__(self, args, logger, model_provider):
        ...
        if args.tune_method == 'adapter':
            adapter_seed_42_path = 'adapter_seed_42.pth'
        ...
        elif args.tune_method == 'lora':
            lora_seed_42_path = 'lora_seed_42.pth'
    ...
    def train(self):
        ...
        if os.path.exists(self.args.output_dir):
            epoch, num_updates, best_metric = self.load_from_last(self.args.output_dir)
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for local_step, batch in enumerate(train_dataloader):
                loss, loss_A_recon, loss_A_mc, loss_L_recon, loss_L_mc, loss_P_recon, loss_P_mc, loss_dist_P = self.train_step(batch)
                self.add_logging(log_dict, 'loss', loss.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_A_recon', loss_A_recon.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_A_mc', loss_A_mc.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_L_recon', loss_L_recon.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_L_mc', loss_L_mc.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_P_recon', loss_P_recon.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_P_mc', loss_P_mc.item() * self.gradient_accumulation_steps)
                self.add_logging(log_dict, 'loss_dist_P', loss_dist_P.item() * self.gradient_accumulation_steps)
    ...
    def valid(self, epoch=0, num_updates=0):
        ...
        with torch.no_grad():
            for local_step, batch in enumerate(valid_dataloader):
                ...
                valid_loss, valid_loss_A_recon, valid_loss_A_mc, valid_loss_L_recon, valid_loss_L_mc, valid_loss_P_recon, valid_loss_P_mc, valid_loss_dist_P, gen_text_adapter, gen_text_lora, gen_text_prefix = self.model.get_valid_loss_and_generation(all_input, decoder_input_ids)           
                self.add_logging(valid_log_dict, 'loss', valid_loss.item())
                self.add_logging(valid_log_dict, 'loss_A_recon', valid_loss_A_recon.item())
                self.add_logging(valid_log_dict, 'loss_A_mc', valid_loss_A_mc.item())
                self.add_logging(valid_log_dict, 'loss_L_recon', valid_loss_L_recon.item())
                self.add_logging(valid_log_dict, 'loss_L_mc', valid_loss_L_mc.item())
                self.add_logging(valid_log_dict, 'loss_P_recon', valid_loss_P_recon.item())
                self.add_logging(valid_log_dict, 'loss_P_mc', valid_loss_P_mc.item())
                self.add_logging(valid_log_dict, 'loss_dist_P', valid_loss_dist_P.item())
    ...
    def test(self, epoch=0, num_updates=0):
        ...
        with torch.no_grad():
            for local_step, batch in enumerate(test_dataloader):
                ...
                test_loss, test_loss_A_recon, test_loss_A_mc, test_loss_L_recon, test_loss_L_mc, test_loss_P_recon, test_loss_P_mc, test_loss_dist_P, gen_text_adapter, gen_text_lora, gen_text_prefix = self.model.get_valid_loss_and_generation(all_input, decoder_input_ids)                
                self.add_logging(test_log_dict, 'loss', test_loss.item())
                self.add_logging(test_log_dict, 'loss_A_recon', test_loss_A_recon.item())
                self.add_logging(test_log_dict, 'loss_A_mc', test_loss_A_mc.item())
                self.add_logging(test_log_dict, 'loss_L_recon', test_loss_L_recon.item())
                self.add_logging(test_log_dict, 'loss_L_mc', test_loss_L_mc.item())
                self.add_logging(test_log_dict, 'test_loss_P_recon', test_loss_P_recon.item())
                self.add_logging(test_log_dict, 'loss_P_mc', test_loss_P_mc.item())
                self.add_logging(test_log_dict, 'loss_dist_P', test_loss_dist_P.item())
    ...
    def load_from_last(self, bs_lr_dir):
        ...
        if not os.path.exists(last_ckpt_path):
            ...
            self.logger.info(f"Loading from {last_ckpt_path}, previous best_mean_performance={best_average_score}")
    ...
    def train_step(self, batch):
        all_input = self.prepare_data(batch)
        loss, loss_A_recon, loss_A_mc, loss_L_recon, loss_L_mc, loss_P_recon, loss_P_mc, loss_dist_P = self.model(all_input)
        loss = loss / self.gradient_accumulation_steps
        # loss.backward(retain_graph=True)
        loss.backward()
        return loss, loss_A_recon, loss_A_mc, loss_L_recon, loss_L_mc, loss_P_recon, loss_P_mc, loss_dist_P
