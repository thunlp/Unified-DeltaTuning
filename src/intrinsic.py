...
class IntrinsicDimensionLight:
    def __init__(self, module: nn.Module, intrinsic_dimension: int,  output_dir,
        ...
        if not (module.share_intrinsic == None):
            # self.intrinsic_parameter = module.share_intrinsic.cpu() if device=="cpu" else module.share_intrinsic.cuda()
            self.intrinsic_parameter = module.share_intrinsic
        else:
        ...
        for name, param in module.named_parameters():
            if 'adapter' not in name and param.requires_grad and (len(str_filter) == 0 or any([x in name for x in str_filter])):
                length += 1
    ...
    def __call__(self, module, inputs, input_ids=None, attention_mask=None, labels=None, decoder_attention_mask=None, only_adapter=False, only_lora=False, only_prefix=False, only_said=True):
        index = 0
        if only_adapter or only_lora or only_prefix:
            if only_said:
                assert False
            else:
                return
