...
def evaluate(predictions, data, metric):
    ...
    if metric == "EM":
        ...
        return_dict[metric] = f1_score([dp[1][0] for dp in data], predictions, average="macro", labels=np.unique([dp[1][0] for dp in data]))
        # return_dict[metric] = f1_score([dp[1][0] for dp in data], predictions, average="macro")
        ...
        return_dict[metric] = pearsonr([float(dp[1][0]) for dp in data], predictions)[0]
